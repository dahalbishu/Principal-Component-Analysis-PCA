...
Y = np.dot( eigenvectors.T,zero_mean_data.T)
...
plt.legend(handles=legend_handles, loc='upper left', bbox_to_anchor=(0.66, 0.999), ncol=len(legend_handles),handlelength=0.3, borderpad=0.08)
plt.show()

#for 1 component
Y = PCAfun([sorted_indices[12]])
...
#for 2 component
Y = PCAfun([sorted_indices[6], sorted_indices[7]])
...
plt.axis('equal')
plt.grid(True)
plt.xlabel('Principal Component 12')
plt.ylabel('Principal Component 13')
plt.title('PCA Scatter Plot')
plt.colorbar()
legend_handles = [
    plt.scatter([], [], label='THA076BEI009\nTHA076BEI036', alpha=0),
...
ax.legend(handles=legend_handles, loc='upper left', bbox_to_anchor=(0.56, 0.94), ncol=len(legend_handles),handlelength=0.3, borderpad=0.08)
plt.show()
...
Y = PCAfun([sorted_indices[7], sorted_indices[8], sorted_indices[9]])
...
ax.scatter(Y[0,:], Y[1,:],Y[2,:],c=y)
ax.set_xlabel('PCA 11')
ax.set_ylabel('PCA 12')
ax.set_zlabel('PCA 13')
ax.set_title('3D Scatter Plot')
ax.set_box_aspect([1, 1, 1])
ax.grid(True)
legend_handles = [
